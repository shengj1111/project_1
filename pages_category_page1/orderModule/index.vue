					...
					<view class="item" v-for="(item, index) in list" :key="index" @click="itemTap(item.orderId,item)">
						<view class="order-list-top">
							<view class="top-l" @click.stop="goShop(item.shopId)">
								...
								<image src="/images/arrowRight.png" class="arrow-img"></image>
							</view>
						...
						<view class="order-info-box">
							<view class="order-info">
								<view class="order-info-item" v-for="(proItem, pIndex) in item.skus" :key="pIndex">
									...
									<view class="info-box">
										<view class="product-name" style="color: red;">
											<view style="display: flex;justify-content: space-between;">
												<text>
													<!-- 支付方式 -->
													<text v-if="item.paymentMode == 1">微信支付</text>
													<text v-if="item.paymentMode == 4">余额支付</text>
													<text v-if="item.paymentMode == 5">线下支付</text>
												</text>
												<text>
													<!-- 配送方式 -->
										...
										<view class="product-name">{{item.createTime}}</view>
										<text class="product-name">{{proItem.productName}}</text>
										<!-- 组合商品 -->
										<zuhe v-if="false"/>
										
										<!-- /组合商品 -->
										<text class="product-name">提货地址:{{item.deliveryAddress}}</text>
										<text class="product-name1" v-if="item.pickupWay===0">收货地址:{{item.receiveAdress}}{{item.address? "("+item.address+")":""}}</text>
								...
								<!-- 金额展示 -->
								<view class="total-price-box">支付金额:¥{{item.price }}
									<text
										v-if="item.yuedeductPrice != ''&&item.yuedeductPrice != 0">,余额抵扣:¥{{item.yuedeductPrice}}</text>
								...
								<!-- 积分展示 -->
								<view class="total-price-box" v-if="item.integral > 0 || item.wuliuIntegral > 0">
									<text v-if="item.integral && item.integral > 0">赠送积分 : {{item.integral}}  </text>
									<text v-if="item.wuliuIntegral && item.wuliuIntegral > 0" style="margin-left: 10rpx;"> 赠送物流积分 : {{item.wuliuIntegral}}</text>
								</view>
							</view>

							<view class="order-btn-box">
								<!-- 	<text class="btn l" v-if='item.state==1|| item.state == 6 '
									@click.stop="cancelOrder(item.orderId,index)">关闭订单</text> -->
								<!-- <text class="btn l" v-if='item.state==2' >提醒发货</text> -->
								<text class="btn r" v-if='item.state==1' @click.stop="payOrder(item,index)">立即付款</text>

								<text class="btn r" v-if='item.state == 2 && item.cancelstate == 0'
									@click.stop="quxiaoOrder(item,index)">取消订单</text>

								<text class="btn r" v-if='item.state == 2 && item.cancelstate == 1'
								...
								<text class="btn r" v-if='item.state==6'
									@click.stop="goInviteSpll(item.collageId,item.orderId,item.skus[0].productId,item.skus[0].skuId,item.shopId)">邀请拼单</text>
								<text class="btn r" v-if='false && item.state==5'
	...
	export default {
		...
		methods: {
			...
			quxiaoOrder(item, index) {
				...
				uni.navigateTo({
					// 
					url: `quxiaoOrder?orderId=${item.orderId}&shopId=${item.shopId}&state=${item.state}`
				})
			},
			//去商品详情
					...
					uni.showModal({
						...
						success: (res) => {
							if (res.confirm) {
								...
								obj.state = 1
								NET.request(API.lijifukuan, obj, 'POST').then(res => {
									uni.showToast({
										title: '已支付',
										duration: 1000,
										icon: 'none'
