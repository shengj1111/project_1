						...
						<scroll-view :scroll-y="true" style="height: 100%">
							<u-cell-group>
								<u-cell-item v-for="(item,index) in provinces" :title="item.label" :arrow="false"
									...
									<u-icon v-if="isChooseP&&province===index" slot="right-icon" size="34"
						...
						<scroll-view :scroll-y="true" style="height: 100%">
							<u-cell-group v-if="isChooseP">
								<u-cell-item v-for="(item,index) in citys" :title="item.label" :arrow="false"
									...
									<u-icon v-if="isChooseC&&city===index" slot="right-icon" size="34"
										name="checkbox-mark"></u-icon>
								</u-cell-item>
							</u-cell-group>
						</scroll-view>
						...
						<scroll-view :scroll-y="true" style="height: 100%">
							<u-cell-group v-if="isChooseC">
								<u-cell-item v-for="(item,index) in areas" :title="item.label" :arrow="false"
									...
									<u-icon v-if="isChooseA&&area===index" slot="right-icon" size="34"
										name="checkbox-mark"></u-icon>
								</u-cell-item>
							</u-cell-group>
						</scroll-view>
...
<script>
	...
	export default {
		...
		methods: {
			init() {
				this.cityValue = ""
				this.isChooseP = false //是否已经选择了省
				this.province = 0 //省级下标
				this.provinces = provinces
				this.isChooseC = false //是否已经选择了市
				this.city = 0 //市级下标
				this.citys = citys[0]
				this.isChooseA = false //是否已经选择了区
				this.area = 0 //区级下标
				this.areas = areas[0][0]
				this.tabsIndex = 0
