...
//获取当前时间标准年月日
timeProto.getLastDate = function (constTime,dateFormat) {
	if (!constTime) {return;}
...
const resDateStr = function (timer,constTime,dateFormat,computeMax) {
	...
	let _dd = function (timer,constTime) {
		...
		if (timer/86400 <= 31) {return Math.ceil(timer/86400) + "天前 " + new Date(constTime).pattern("HH:mm")}
		else return 'next';
	}
	let _dlast = function (timer,constTime,dateFormat) {
		return _time_.getLastDate(constTime,dateFormat);
	}
	let dateFilter;
	if (computeMax == 'mm') {
		dateFilter = _just.asyAfter(_mm).asyAfter(_dlast);
	}
	else if (computeMax == 'hh') {
		dateFilter = _just.asyAfter(_mm).asyAfter(_hh).asyAfter(_dlast);
	}
	else {
		dateFilter = _just.asyAfter(_mm).asyAfter(_hh).asyAfter(_dd).asyAfter(_dlast);
	}
	return dateFilter(timer,constTime,dateFormat);
}


//转换时间
const reg=new RegExp("-","g");
timeProto.getFormatTime = function (constTime,max,dateFormat,computeMax) {
	if (!constTime) {return "";}
	...
	if (constTime > now && max) {
		return this.getLastDate(constTime,dateFormat);
	}
	
	let _t = this;
	return resDateStr(timer,constTime,dateFormat,computeMax);
}
